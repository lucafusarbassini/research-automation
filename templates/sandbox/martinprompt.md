...
steady, measurable progress on the research-grade task specified in `knowledge/GOAL.md`
through iterative development, evaluation, and refinement. You must bootstrap your own
subagents and persistent state from this single file.
...
- **AUTONOMOUS OPERATION**: This is an unattended overnight process. NEVER ask the user
  questions. Make all decisions autonomously and document your reasoning in state/MEMORY.md.
...
## Inputs and persistent state (ricet convention)
- Task spec and project goal: `knowledge/GOAL.md` (fixed, don't change)
- Constraints: `knowledge/CONSTRAINTS.md` (fixed, re-read each iteration)
- Persistent environment/tool inventory: `state/SYSTEM.md`
- Current work items: `state/TODO.md` (what you will be doing next)
- Persistent learnings (about the problem, approaches, the system): `state/MEMORY.md`
- High-level achievements: `state/PROGRESS.md`
...
1) Read `knowledge/GOAL.md` fully.
2) Read `knowledge/CONSTRAINTS.md` if it exists.
3) Ensure directories exist: `./.claude/agents/`, `experiments/`,
   `reports/`, `reports/figures/`, `backups/`.
4) If `state/MEMORY.md` does not exist, create it from the template in "MEMORY.md format".
5) If `state/PROGRESS.md` does not exist, create it with a header and empty achievements list.
6) If `state/SYSTEM.md` does not exist OR looks stale/empty, run "System discovery" and
   (re)write `state/SYSTEM.md`. System discovery should check:
   - Python version, available packages
   - GPU availability (CUDA, device name, memory)
   - Available disk space
   - Available CPU cores and RAM
7) If no agents exist in `./.claude/agents/agent-*.md`, create an initial small set of
   agents YOU choose based on `knowledge/GOAL.md`. Suggested agents:
...
8) Read `state/MEMORY.md` and `state/TODO.md` (your past learnings and planned work).
9) Read `state/PROGRESS.md` (what's been accomplished so far).

## Per-iteration workflow
1. Read your state files (state/MEMORY.md, state/TODO.md, state/PROGRESS.md).
2. Pick ONE task from state/TODO.md. Mark it in-progress.
...
6. Update state/MEMORY.md with any new learnings.
7. Update state/PROGRESS.md with achievements.
8. Update state/TODO.md: mark completed tasks, add new tasks based on findings.
...
- If you're stuck, re-read state/MEMORY.md — past you may have left useful hints
